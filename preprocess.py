...
def main(args):
    ...
    test_df = pd.DataFrame({'ID': files[(train_data_num+valid_data_num):all_data_num], 'Type': 'test'})
    train_csv_path = opj(cfg.csv_dir, 'train_list.csv')
    valid_csv_path = opj(cfg.csv_dir, 'valid_list.csv')
    test_csv_path = opj(cfg.csv_dir, 'test_list.csv')
